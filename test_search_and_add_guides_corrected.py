...
def test_search_existing_guides_corrected():
    """
    ...
    assert "Self-Installation Guide for Your NetMaster XG6 Modem" in result["answer"]

def test_search_after_adding_pdf_corrected():
    """
    ...
    assert "Custom Guide" in result["answer"]
    assert "custom guide" in result["answer"]

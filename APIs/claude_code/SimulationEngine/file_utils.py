...
def read_file(file_path: str, max_size_mb: int = 50) -> Dict[str, Union[str, int]]:
    """
    ...
    Returns:
        Dict[str, Any]: A dictionary containing:
            - content (str): The file content (text or base64 encoded string)
            - encoding (str): The encoding type ('text' or 'base64')
            - mime_type (str): The detected MIME type of the file
...
def _is_within_workspace(path: str, workspace_root: str) -> bool:
    """Check if a path is within the workspace, preventing directory traversal."""
    try:
        # Normalize both paths to be safe
...
def _create_parent_directories(dir_path: str, fs: Dict, workspace_root: str):
    """Recursively create parent directory entries in the file system DB.

    Args:
        dir_path (str): The path to the directory to create.
        fs (Dict): The file system DB.
        workspace_root (str): The workspace root.
    """
    if dir_path == workspace_root or dir_path in fs:
        return
...
def _is_ignored(path: str, workspace_root: str, respect_git_ignore: bool = True) -> bool:
    """Check if a file or directory should be ignored.
    This includes checking default ignore patterns and .gitignore files.
    """
    if respect_git_ignore:
        ...
        if os.path.exists(gitignore_path):
            with open(gitignore_path, 'r') as f:
                ...
                # Add workspace root to gitignore patterns to make them relative to the root
                gitignore_patterns = [os.path.join(workspace_root, p) for p in gitignore_patterns]
                if _should_ignore(path, gitignore_patterns):
                    return True

    # Add other ignore logic here if needed, e.g., from a config file
    return False

"""
Simple File Utilities for Messages Service
...
def write_file(file_path: str, content: Union[str, bytes], encoding: str = 'text') -> None:
    ...
    if encoding == 'base64':
        ...
        # Expect bytes; if string provided, treat as base64 for convenience
        if isinstance(content, bytes):
            content_bytes = content
        elif isinstance(content, str):
            content_bytes = base64.b64decode(content)
        else:
            raise TypeError("content must be bytes or base64 string when encoding='binary'")
        with open(file_path, 'wb') as f:
            f.write(content_bytes)
    elif encoding == 'text':
        # Write as text
        ...
        raise ValueError("Unsupported encoding. Use 'text', 'binary', or 'base64'.")

...
from ..SimulationEngine.custom_errors import NoMediaPlayerError, NoPlaylistError, NoMediaItemError, InvalidPlaybackStateError
...
class TestPrevious(BaseTestCaseWithErrorHandler):
    ...
    def test_previous_first_track(self):
        ...
        self.assertEqual(result["result"], "Success")
        self.assertEqual(result["app_name"], "Spotify")
        self.assertEqual(result["title"], self.media_item1.title)
        
        # Verify the track was restarted
        player_state = DB["media_players"]["Spotify"]
        self.assertEqual(player_state["current_media"]["current_position_seconds"], 0)
        self.assertEqual(player_state["playback_state"], "PLAYING")

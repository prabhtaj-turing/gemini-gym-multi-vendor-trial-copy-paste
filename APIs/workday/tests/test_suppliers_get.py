...
class TestSupplierCompaniesGet:
    """Comprehensive tests for Suppliers.get_suppliers function following API documentation."""
    ...
    def test_get_response_structure_compliance(self):
        ...
        response, status = WorkdayStrategicSourcingAPI.Suppliers.get_suppliers()
        assert status == 200
    ...
    def test_include_empty_string(self):
        ...
        response, status = WorkdayStrategicSourcingAPI.Suppliers.get_suppliers(_include="")
        assert status == 200
        assert "included" not in response

    def test_include_attachments(self):
        """Test including attachments."""
        response, status = WorkdayStrategicSourcingAPI.Suppliers.get_suppliers(_include="attachments")
        assert status == 200
    ...
    def test_filter_by_name(self):
        ...
        response, status = WorkdayStrategicSourcingAPI.Suppliers.get_suppliers(
            filter={"name": "Supplier A"}
        )
        assert status == 200
        assert len(response["data"]) == 1
        assert response["data"][0]["attributes"]["name"] == "Supplier A"
    ...
    def test_filter_no_matches(self):
        ...
        response, status = WorkdayStrategicSourcingAPI.Suppliers.get_suppliers(
            filter={"name": "Nonexistent Supplier"}
        )
        assert status == 200
        assert response["data"] == []
        assert response["meta"]["count"] == 1
    ...
    def test_invalid_filter_parameter_type(self):
        ...
        response, status = WorkdayStrategicSourcingAPI.Suppliers.get_suppliers(filter="invalid")
        assert status == 400
    ...
    def test_invalid_include_parameter_type(self):
        ...
        response, status = WorkdayStrategicSourcingAPI.Suppliers.get_suppliers(_include=123)
        assert status == 400
        assert "errors" in response
    ...
    def test_invalid_page_parameter_type(self):
        ...
        response, status = WorkdayStrategicSourcingAPI.Suppliers.get_suppliers(page="invalid")
        assert status == 400
        assert "errors" in response
        assert "must be a dictionary" in response["errors"][0]["detail"]
...
def test_get_with_filter():
    ...
    db.DB["suppliers"]["supplier_companies"][1] = {
        "id": 1,
        "attributes": {"name": "Company 1", "segmentation_status": "approved"}
    }

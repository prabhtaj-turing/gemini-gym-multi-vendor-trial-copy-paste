"""
Test suite for CES Account Management Infobot Configuration Integration

Tests the integration of the new configuration system with ces_account_management service.
"""
...
from APIs.ces_account_management.SimulationEngine import utils
from APIs.ces_account_management.SimulationEngine.utils import (
...
class TestInfobotConfigIntegration(BaseTestCaseWithErrorHandler):
    ...
    @patch('APIs.ces_account_management.SimulationEngine.utils.service_account')
    @patch('APIs.ces_account_management.SimulationEngine.utils.requests.Session')
    ...
    @patch('APIs.ces_account_management.SimulationEngine.utils._get_token')
    @patch('APIs.ces_account_management.SimulationEngine.utils.requests.post')
    def test_query_infobot_success(self, mock_post, mock_get_token):
        ...
        result = utils._query_infobot('test query', 'account_orders')
    ...
    def test_query_infobot_missing_tool_resource(self):
        ...
        self.assert_error_behavior(
            ...
            ValueError,
            f"Tool resource '{tool_name}' not configured",
            query=query,
            tool_name=tool_name,
        )
    ...
    # Tests for query_plans_and_features_infobot
    @patch('APIs.ces_account_management.SimulationEngine.utils._query_infobot')
    def test_query_plans_and_features_infobot(self, mock_query_infobot):
        """Test query_plans_and_features_infobot calls _query_infobot with correct tool name."""
        expected_response = {'answer': 'Plans and features response'}
        mock_query_infobot.return_value = expected_response
        
        result = utils.query_plans_and_features_infobot('What plans are available?')
    ...
    # Tests for query_account_orders_infobot
    @patch('APIs.ces_account_management.SimulationEngine.utils._query_infobot')
    def test_query_account_orders_infobot(self, mock_query_infobot):
        """Test query_account_orders_infobot calls _query_infobot with correct tool name."""
        expected_response = {'answer': 'Account orders response'}
        mock_query_infobot.return_value = expected_response
        
        result = utils.query_account_orders_infobot('What is my order status?')
    ...
    def test_tool_resources_customization(self):
        ...
        custom_tools = {
            'account_orders': 'custom-orders-id',
            'plans_features': 'custom-plans-id',
        ...
        self.assertEqual(config.tool_resources['account_orders'], 'custom-orders-id')
        self.assertEqual(config.tool_resources['new_tool'], 'new-tool-id')


class TestInfobotConfigDefaults(unittest.TestCase):
    """Test that default configuration values are correct for ces_account_management"""
    ...
    def test_default_tool_resources(self):
        ...
        self.assertIn('account_orders', config.tool_resources)
        self.assertIn('plans_features', config.tool_resources)
        self.assertEqual(config.tool_resources['account_orders'], '46f527f8-0509-4e28-9563-db5666e0790b')
        self.assertEqual(config.tool_resources['plans_features'], 'c90c11bb-6868-4631-8bf0-7f8b5fe4b92c')
